...
def get_k_run_starter(n, k):
    ...
    i = 0
    final = None
    while n // 10 != 0 and k >= 0:
        while n % 10 <= (n // 10) % 10:  # 检查当前数字是否大于前一个数字
            n //= 10
            if n // 10 == 0:
                break  # 避免无限循环，确保最后一位数字被处理
        if n // 10 == 0 and k == 0:
            final = n  # 对于最后一次循环，更新 'final' 变量
        i += 1  # 移动到下一个运行
        n //= 10  # 更新 n 以跟踪当前运行的索引
        k -= 1  # 更新 k 以跟踪当前运行的索引
    return final
...
def nearest_two(x):
    ...
    if x >= 1.0:
        ...
        cheng = 0.5
    while abs(power_of_two - x) > abs(power_of_two * cheng - x):
        power_of_two *= cheng
    if abs(power_of_two - x) == abs(power_of_two * cheng - x) and x > 1:
